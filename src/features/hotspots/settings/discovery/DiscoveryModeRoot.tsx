...
const DiscoveryModeRoot = ({ onClose, hotspot: propsHotspot }: Props) => {
  ...
  const fetchRecent = useCallback(() => {
    ...
    dispatch(fetchRecentDiscoveries({ deviceAddress: hotspot.address }))
  ...
  useEffect(() => {
    ...
    const lat = Number(Config.DISCO_DEVICE_LAT)
    const lng = Number(Config.DISCO_DEVICE_LNG)
    const address = Config.DISCO_DEVICE_ADDRESS
    const name = Config.DISCO_DEVICE_NAME
  ...
  const handleNewSelected = useCallback(async () => {
    ...
    dispatch(startDiscovery({ deviceAddress: hotspot.address }))
  }, [dispatch, hotspot.address, userAddress])
